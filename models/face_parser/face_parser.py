...
class FaceParser(nn.Module):
    def __init__(self, cfg):
        ...
        self.mask_idxs = {'mse': [self.mask2idx['nose']],
                          'blend': [self.mask2idx['skin']],
                          'shape': [self.mask2idx['skin']],
                          'dynamic': []}
    ...
    def set_idx(self, attribute):
        ...
        if attribute in ['wearing_lipstick', 'mouth_slightly_open', 'smiling', 'big_lips', 'Mouth Closed', 'Smiling', 'Big Lips']:
            target_name = 'mouth'
        elif attribute in ['bushy_eyebrows', 'arched_eyebrows', 'Bushy Eyebrows', 'Arched Eyebrows']:
            target_name = 'eyebrows'
        elif attribute in ['narrow_eyes', 'Brown Eyes']:
            target_name = 'eyes'
        elif attribute in ['pointy_nose', 'big_nose', 'Big Nose', 'Pointy Nose']:
            target_name = 'nose'
        elif attribute in ['black_hair', 'brown_hair', 'blond_hair', 'gray_hair', 'wavy_hair', 'straight_hair',
        'Wavy Hair', 'Receding Hairline', 'Bangs',
        'Black Hair', 'Blond Hair', 'Brown Hair', 'Gray Hair']:
            target_name = 'hair'
        elif attribute in ['Oval Face','Square Face','Round Face', 'Chubby', 'Obstructed Forehead',
        ...
        #self.mask_idxs['mse'] += [3]

        self.mask_idxs['blend'] += [target_idx]

            
        #self.mask_idxs['shape'] += [target_idx]
        #self.mask_idxs['shape'] += [1]
        #self.mask_idxs['shape'] += [2]
        #self.mask_idxs['shape'] += [3]
        #self.mask_idxs['shape'] += [4]
        #self.mask_idxs['shape'] += [5]
        self.mask_idxs['shape'] += [6]
        #self.mask_idxs['shape'] += [7]
        #self.mask_idxs['shape'] += [8]
        
        self.mask_idxs['dynamic'] += [target_idx]

        if attribute in ('straight_hair', 'wavy_hair',
        'Wavy Hair', 'Receding Hairline', 'Bangs'
        ):  # hair shape requires careful treatment, especially with ear component
