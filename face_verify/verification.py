...
def main(args):
    ...
    for data in tqdm(datas):
        try:
            ...
            #path1 = os.path.join('/exdata2/RuoyuChen/face_verify/Verification/dataset/VGGFace2-test/',data.split(' ')[0])
            #path2 = os.path.join('/exdata2/RuoyuChen/face_verify/Verification/dataset/VGGFace2-test/',data.split(' ')[1])
            path1 = data.split(' ')[0]
            path2 = data.split(' ')[1]
            ...
            dist = 1 - np.arccos(similar)/math.pi
            with open(args.save_List, 'a') as file:
                file.write(path1+' '+path2+' '+str(dist)+' '+data.split(' ')[2]+'\n')
        except:
            pass
...
def parse_args():
    ...
    parser.add_argument('--List',
                        type=str,
                        default='./text/tutorial_interfacegan_id.txt',
                        help='Datasets.')
    parser.add_argument('--save_List',
                        type=str,
                        default='./results/text/ArcFace-r100-tutorial_san_id.txt',
                        help='Datasets.')
