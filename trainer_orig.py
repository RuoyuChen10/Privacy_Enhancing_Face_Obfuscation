...
class Trainer():
    ...
    def train_latent(self):
        ...
        for i in range(self.start_steps, self.cfg.N_LATENT_STEPS):
            ...
            #print("mask_I_gen.shape",mask_I_gen.shape)              
            ...
            if self.cfg.LOSS.weights.classf > 0 and i >= self.cfg.LOSS.start_steps.classf:
                ...
                if attr_pred_orig_1>= 0.5:               
                   loss['classf'] = 3*kl_divergence(1.0-attr_pred_1, self.target)
                else:
                   loss['classf'] = kl_divergence(attr_pred_1, self.target)
            ...
            if self.cfg.LOSS.weights.seg > 0 and i >= self.cfg.LOSS.start_steps.seg:
                i_dynamic_mask = self.models.face_parser(images_generated, mode='mse')
                loss['seg'] = batch_mse_loss(i_dynamic_mask, self.masks['mse'])
