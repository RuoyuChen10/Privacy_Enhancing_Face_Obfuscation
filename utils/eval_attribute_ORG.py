...
def parse_args():
    ...
    parser.add_argument('--test-set', type=str, default='/home/lsf/桌面/MaskFaceGAN/utils/attribute2_orig.txt')      # 属性测试集的list
...
def main(args):
    ...
    for data in tqdm(datas):
        ...
        #file_img = data.split(" ")[0]
        #if file_img.split('/')[0] in Id_file.keys():
             #Id_im = Id_file[file_img.split('/')[0]]
        #else:
             #continue

        #im_name = file_img.split('/')[1]
        #img_name = im_name.split('.')[0]+'-'+str(Id_im)+'.jpg'
