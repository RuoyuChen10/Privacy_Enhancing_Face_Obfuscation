...
def parse_args():
    ...
    parser.add_argument('--image-dir', type=str, default='/home/lsf/桌面/MaskFaceGAN-github/results-com/vggface_output/')    # VGGFace2数据集的文件夹，这个放置固定的
    parser.add_argument('--attribute-set', type=str, default='/exdata2/RuoyuChen/Datasets/VGGFace2/attribute')      # 属性标签
    parser.add_argument('--test-set', type=str, default='/home/lsf/桌面/MaskFaceGAN/utils/attribute2.txt')      # 属性测试集的list
    parser.add_argument('--json-path', type=str, default='/home/lsf/桌面/MaskFaceGAN/json_per_ima')      # json path
...
def convert_(args, image_names, type_="train.txt"):
...
def convert(args):
    ...
    # 读取数据集
    convert_(args, image_names, type_="train.txt")
    convert_(args, image_names, type_="test.txt")
    convert_(args, image_names, type_="val.txt")
...
def main(args):
    ...
    for data in tqdm(datas):
        ...
        #    args.image_dir, im_name.split(".")[0]+"-0.8.jpg"
        #)
        #print(image_path)        
        ...
        if args.attribute in Gender:
            ...
            if predicted_truth == gt_judge and predicted_truth:
        ...
        if number == args.test_number:
            break
    print("测试结束，有效测试数据数量为{}张，属性{}的预测结果的准确率为{}".format(number, args.attribute, acc/number))
