...
from models import BranchedTinyAttr, FaceParser, Generator, AttributeNet, FaceNet
...
class ModelsModule(nn.Module):
    ...
    def get_image_masks(self, images):
        ...
        masks = {}
        print("self.face_parser.mask_modes", self.face_parser.mask_modes)
...
class NewModelsModule(nn.Module):
    def __init__(self, cfg, attribute_subset=None):
        super().__init__()
        self.cfg = cfg
        self.update_shape = cfg.SEGMENTATOR.UPDATE_SHAPE

        self.classifier = AttributeNet()
        self.face_parser = FaceParser(cfg.SEGMENTATOR)
        self.generator = Generator(1024, 512, 8)
        self.face_net = FaceNet(cfg.FACENET.Net, cfg.FACENET.TOP)
